...
async def create_manual_activity(
    ...
    # Step 1
    activity = await service.insert_manual(request)
    ...
    refreshed = await get_activity_or_404(
        session,
        activity.id,
    )

    return {
        "data": to_activity_response(
            refreshed
        ).model_dump()
    }
...
async def retry_activity(
    ...
    """Retry a failed activity from its current stage."""

    activity = await get_activity_or_404(
        session,
        activity_id,
    )
